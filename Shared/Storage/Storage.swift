...
internal struct Storage {
    ...
    internal static var currentDB : Database?
    ...
    internal static func storeDatabase(_ db : Database, context : NSManagedObjectContext?, newElements : [DatabaseContent<Date>]) throws -> Void {
        ...
        let database : EncryptedDatabase = try encrypter.encryptDatabase(db)
        var encryptedElements : [DatabaseContent<Data>] = []
        for element in newElements {
            let encrypted : DatabaseContent<Data>
            switch element {
            case is Entry:
                encrypted = try encrypter.encryptEntry(element as! Entry)
            case is Folder:
                encrypted = try encrypter.encryptFolder(element as! Folder)
            case is DB_Document:
                encrypted = try encrypter.encryptDocument(element as! DB_Document)
            case is DB_Image:
                encrypted = try encrypter.encryptImage(element as! DB_Image)
            default:
                continue
            }
            encryptedElements.append(encrypted)
        }
        ...
        case .CoreData:
            ...
            try CoreDataManager.storeDatabase(database,
                                              context: context!,
                                              newElements: encryptedElements
            )
        case .File:
            try DatabaseFileManager.storeDatabase(database, newElements: encryptedElements)
        }
    }
